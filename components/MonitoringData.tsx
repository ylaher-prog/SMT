...
import { PlusIcon, PencilIcon, TrashIcon, ArrowsUpDownIcon, ArrowUpIcon, ArrowDownIcon, PaperClipIcon } from './Icons';
...
type SortableKey = 'type' | 'educator' | 'phase' | 'date' | 'status' | 'priority' | 'score';

const MonitoringData: React.FC<MonitoringDataProps> = (props) => {
    const { teachers, observations, setObservations, academicStructure, phaseStructures, monitoringTemplates, currentAcademicYear, classGroups, allocations, currentUser, currentTenantId } = props;
    ...
    const sortedAndFilteredObservations = useMemo(() => {
        ...
        if (sortConfig) {
            filtered.sort((a, b) => {
                ...
                else if (sortConfig.key === 'score') { aValue = a.calculatedScore ?? -1; bValue = b.calculatedScore ?? -1; }
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                        <thead className="bg-gray-50 dark:bg-slate-700/50">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider"><div className="flex items-center gap-1 cursor-pointer" onClick={() => requestSort('score')}>Score {getSortIcon('score')}</div></th>
                                ...
                                <tr key={obs.id} className="hover:bg-gray-50 dark:hover:bg-slate-800">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            {obs.evidenceFiles && obs.evidenceFiles.length > 0 && <PaperClipIcon className="w-4 h-4 text-gray-400 flex-shrink-0" />}
                                            <div>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-800 dark:text-gray-300">
                                        {obs.calculatedScore ? `${(obs.calculatedScore * 100).toFixed(1)}%` : 'N/A'}
                                    </td>
