...
const BulkImportModal: React.FC<BulkImportModalProps> = ({ isOpen, onClose, setTeachers, academicStructure, teachers, currentTenantId }) => {
    ...
    const validatedData: ParsedTeacherRow[] = useMemo(() => {
        ...
        const existingTeacherEmailMap = new Map(teachers.map(t => [t.email.toLowerCase(), t]));
        const existingTeacherCodeMap = new Map(teachers.map(t => t.employeeCode ? [t.employeeCode.toLowerCase(), t] : [null, null]).filter(e => e[0] !== null) as [string, Teacher][]);
        ...
        const newTeacherEmails = new Set(parsedItems.filter(p => !p.error).map(p => p.email!.toLowerCase()));
        const newTeacherNames = new Set(parsedItems.filter(p => !p.error).map(p => p.fullName!.toLowerCase()));
        const allFullNames = [...teachers.map(t => t.fullName.toLowerCase()), ...parsedItems.filter(p => !p.error).map(p => p.fullName!.toLowerCase())];
        ...
        return parsedItems.map((item, index) => {
            ...
            if (item.employeeCode) {
                existingTeacher = existingTeacherCodeMap.get(item.employeeCode.toLowerCase());
            }
            if (!existingTeacher && item.email) {
                existingTeacher = existingTeacherEmailMap.get(item.email.toLowerCase());
            }

            if (item.managerEmail && !existingTeacherEmailMap.has(item.managerEmail.toLowerCase()) && !newTeacherEmails.has(item.managerEmail.toLowerCase())) return { ...item, error: `Manager email "${item.managerEmail}" not found on line ${index + 2}.` };
            ...
            if (item.managerName && !newTeacherNames.has(item.managerName.toLowerCase()) && !teachers.some(t => t.fullName.toLowerCase() === item.managerName!.toLowerCase())) return { ...item, error: `Manager name "${item.managerName}" not found.` };

            return existingTeacher ? { ...item, isUpdate: true, existingTeacherId: existingTeacher.id } : item;
        });
    }, [csvData, academicStructure, teachers, positionNameToIdMap]);
    
    const importStats = useMemo(() => {
        return {
            toCreate: validatedData.filter(d => !d.error && !d.isUpdate).length,
            toUpdate: validatedData.filter(d => !d.error && d.isUpdate).length,
            errors: validatedData.filter(d => d.error).length,
        };
    }, [validatedData]);

    const handleImport = () => {
        if (importStats.errors > 0) { setError('Cannot import. Please fix the errors shown in the preview.'); return; }
        if (importStats.toCreate === 0 && importStats.toUpdate === 0) { setError('No valid records to import.'); return; }

        const toCreate = validatedData.filter(d => !d.error && !d.isUpdate);
        const toUpdate = validatedData.filter(d => !d.error && d.isUpdate);

        const newTeachersWithIds = toCreate.map(item => ({ ...item, id: `t${Date.now()}${Math.random()}` }));
        ...
        // FIX: Add tenantIds to the new teacher object to satisfy the Teacher type.
        const newTeachers: Teacher[] = newTeachersWithIds.map(item => {
            ...
            return {
                ...
                markingTasks: 0, slas: { messageResponse: 0, markingTurnaround: 0 }, username, passwordHash, tenantIds: [currentTenantId] };
        });
        
        const teacherUpdates = new Map<string, Partial<Teacher>>();
        toUpdate.forEach(item => {
            ...
            const updateData: Partial<Teacher> = {
                fullName: item.fullName!, email: item.email!, employeeCode: item.employeeCode!, employmentStatus: item.employmentStatus!,
                startDate: item.startDate!, positionId: item.positionId!, managerId, specialties: item.specialties!,
                preferredGrades: item.preferredGrades!, maxLearners: item.maxLearners!, maxPeriodsByMode: item.maxPeriodsByMode!
            };
            teacherUpdates.set(item.existingTeacherId!, updateData);
        });

        setTeachers(prev => {
            const updatedExisting = prev.map(t => teacherUpdates.has(t.id) ? { ...t, ...teacherUpdates.get(t.id) } : t);
            return [...updatedExisting, ...newTeachers];
        });
        
        setImportResult({ created: createdCredentials, updated: toUpdate.length });
    };
    ...
    const handleDownloadTemplate = () => {
        ...
        const example1 = "\"John Doe\",john.doe@example.com,EMP001,Permanent,2023-09-01,\"Mathematics;Physics\",\"10;11\",\"Senior Teacher\",250,\"Live=18;Flipped Afternoon=5\",manager.jane@example.com,";
        const example2 = "\"Jane Smith\",jane.smith@example.com,EMP002,Probation,2024-02-15,\"English;History\",\"9\",\"Junior Teacher\",220,\"Live=20;Self-Paced=2\",,John Doe";
    ...
    const handleExportCredentials = () => {
        ...
        const csvRows = importResult.created.map(cred => `"${cred.fullName}","${cred.username}","${cred.tempPassword}"`);
        ...
        return (
            <Modal isOpen={isOpen} onClose={handleClose} title="Import Successful" size="lg">
                <div className="text-center p-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white">{importResult.created.length} new accounts created & {importResult.updated} records updated!</h3>
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="Bulk Import Teachers" size="xl">
      <div className="space-y-4">
        <div>
            ...
            <ul className="list-disc list-inside text-sm mt-2 bg-gray-50 dark:bg-slate-700/50 p-3 rounded-md"><li>This tool will **update** existing teachers if a matching `EmployeeCode` or `Email` is found, otherwise it will **create** a new teacher.</li><li>Required columns: `FullName`, `Email`, `EmploymentStatus`, `Position`.</li><li>`EmployeeCode` is the primary key for matching and is crucial for linking salary data.</li><li>Separate multiple values (`Specialties`) with a semicolon (`;`).</li><li>Provide `ManagerEmail` or `ManagerName` to assign a manager. Email is preferred for accuracy.</li></ul>
        </div>
        <FileUpload onFileRead={setCsvData} />
        {csvData.trim() && (
            <div>
                 <h4 className="font-semibold mb-2">Preview ({importStats.toCreate} to create, {importStats.toUpdate} to update, {importStats.errors} errors)</h4>
                 <div className="max-h-60 overflow-y-auto border dark:border-slate-600 rounded-md">
                    <table className="min-w-full divide-y text-sm"><thead className="bg-gray-50 dark:bg-slate-700"><tr><th className="px-4 py-2 text-left font-medium">Name</th><th className="px-4 py-2 text-left font-medium">Position</th><th className="px-4 py-2 text-left font-medium">Status</th></tr></thead>
                        <tbody className="bg-white dark:bg-transparent divide-y">
                           {validatedData.map((item, index) => (<tr key={index} className={item.error ? 'bg-red-50 dark:bg-red-900/20' : ''}>
                                ...
                                : (<>
                                    ...
                                    <td className="px-4 py-2">{academicStructure.positions.find(p => p.id === item.positionId)?.name}</td>
        ...
        <div className="flex justify-end gap-4 pt-4">
            ...
            <button type="button" onClick={handleImport} disabled={importStats.errors > 0 || (importStats.toCreate === 0 && importStats.toUpdate === 0)} className="bg-brand-accent text-white px-4 py-2 rounded-md font-semibold hover:bg-amber-600 disabled:bg-gray-400">Import {importStats.toCreate + importStats.toUpdate} Records</button>
        </div>
      </div>
    </Modal>
  );
