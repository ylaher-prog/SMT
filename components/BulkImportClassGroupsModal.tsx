...
const BulkImportClassGroupsModal: React.FC<BulkImportClassGroupsModalProps> = ({ isOpen, onClose, academicStructure, classGroups, setClassGroups, currentTenantId }) => {
    ...
    const existingGroupMap = useMemo(() => new Map(classGroups.map(cg => [`${cg.name.toLowerCase()}|${cg.academicYear}`, cg])), [classGroups]);
    // FIX: Add curriculum name to ID map
    const curriculumNameMap = useMemo(() => new Map(academicStructure.curricula.map(c => [c.name.toLowerCase(), c.id])), [academicStructure.curricula]);
    ...
    const parsedData: ParsedGroup[] = useMemo(() => {
        ...
        const h = {
            ...
            addToTimetable: headerLine.indexOf('addtotimetable'),
        };
        ...
        return dataLines.map((line, index) => {
            ...
            const curriculumName = columns[h.curriculum];
            ...
            if (!name || !academicYear || !curriculumName || !grade || !mode || !learnerCountStr || !subjectsStr) {
            ...
            if (!curriculumId) {
                return { error: `Line ${index + 2}: Curriculum "${curriculumName}" not found.` } as ParsedGroup;
            }
            ...
            const compositeKey = `${name.toLowerCase()}|${academicYear}`;
            const existingGroup = existingGroupMap.get(compositeKey);

            return { 
                name, academicYear, curriculumId, grade, mode, learnerCount, subjectNames, subjectIds, addToTimetable,
                isUpdate: !!existingGroup,
                existingGroupId: existingGroup?.id,
            };
        });
    }, [csvData, academicStructure, subjectNameMap, existingGroupMap, curriculumNameMap]);
    ...
    const handleImport = () => {
        ...
        // FIX: Add tenantId to new class group objects to satisfy the ClassGroup type.
        const newClassGroups: ClassGroup[] = toCreate.map(item => ({
        ...
        const groupUpdates = new Map<string, Partial<ClassGroup>>();
        toUpdate.forEach(item => {
            const updateData: Partial<ClassGroup> = {
                name: item.name,
                academicYear: item.academicYear,
                curriculumId: item.curriculumId,
                grade: item.grade,
            ...
            groupUpdates.set(item.existingGroupId!, updateData);
        });

        setClassGroups(prev => {
            const updatedExisting = prev.map(cg => groupUpdates.has(cg.id) ? { ...cg, ...groupUpdates.get(cg.id) } : cg);
            return [...updatedExisting, ...newClassGroups];
        });

        alert(`${importStats.toCreate} groups created, ${importStats.toUpdate} groups updated.`);
        onClose();
    };

    const handleDownloadTemplate = () => {
        const headers = "name,academicYear,curriculum,grade,mode,learnerCount,subjects,addToTimetable";
        const example1 = "\"Grade 10 Science A\",2024,British,10,Live,25,\"Mathematics;Physics;English\",yes";
        const example2 = "\"Grade 11 Cambridge Bio\",2024,Cambridge,11,Flipped,22,\"Biology;Chemistry\",no";
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <ul className="list-disc list-inside text-sm text-brand-text-light dark:text-gray-400 mt-2 bg-gray-50 dark:bg-slate-700/50 p-3 rounded-md">
                        <li>This tool will **update** existing groups if a matching `name` and `academicYear` is found, otherwise it will **create** a new group.</li>
                                       ...
                                       <tr key={index} className={item.error ? 'bg-red-50 dark:bg-red-900/20' : ''}>
                                           {item.error ? (
                                               ...
                                               <>
                                                ...
                                                <td className="px-4 py-2 dark:text-gray-300">{item.academicYear} - {item.grade} - {academicStructure.curricula.find(c => c.id === item.curriculumId)?.name}</td>
                ...
                <div className="flex justify-end gap-4 pt-4">
                    ...
                    >
                        Import {importStats.toCreate + importStats.toUpdate} Groups
                    </button>
                </div>
            </div>
        </Modal>
