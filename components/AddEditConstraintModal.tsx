...
const AddEditConstraintModal: React.FC<AddEditConstraintModalProps> = (props) => {
    const { isOpen, onClose, onSave, existingConstraint, teachers, currentAcademicYear, currentTenantId } = props;
    ...
    useEffect(() => {
        if (existingConstraint) {
            ...
            // FIX: Add tenantId to the state object to satisfy the TimeConstraint type.
            setFormState({
                type: 'teacher-max-periods-day',
                teacherId: teachers[0]?.id || '',
                maxPeriods: 5,
    ...
    }, [isOpen, existingConstraint, teachers, currentTenantId]);
    ...
    const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        switch(newType) {
            case 'teacher-max-periods-day':
                // FIX: Add tenantId to the state object to satisfy the TimeConstraint type.
                setFormState({ type: newType, teacherId: teachers[0]?.id || '', maxPeriods: 5, tenantId: currentTenantId });
                break;
            case 'teacher-max-consecutive':
                // FIX: Add tenantId to the state object to satisfy the TimeConstraint type.
                setFormState({ type: newType, teacherId: teachers[0]?.id || '', maxPeriods: 3, tenantId: currentTenantId });
                break;
        }
    };
