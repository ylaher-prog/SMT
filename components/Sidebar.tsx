...
import { ChartPieIcon, UsersIcon, ClipboardDocumentListIcon, CalendarDaysIcon, EyeIcon, ShoppingCartIcon, Cog6ToothIcon, ChatBubbleLeftRightIcon, CurrencyDollarIcon, TableCellsIcon, BriefcaseIcon, ClipboardDocumentCheckIcon, AdjustmentsHorizontalIcon, UserPlusIcon, ArchiveBoxIcon } from './Icons';
...
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage, currentUser, academicStructure, permissions }) => {
  const isHrSubmenuActive = ['payroll', 'leave', 'recruitment'].includes(activePage);
  ...
  const navItems = [
    ...
    { id: 'workflow', label: 'Workflow', icon: <AdjustmentsHorizontalIcon />, permission: 'view:workflow' },
  ];
  
  const hrNavItems = [
    { id: 'recruitment', label: 'Recruitment', icon: <UserPlusIcon />, permission: 'view:recruitment' },
  ...
  const otherNavItems = [
    ...
    { id: 'assets', label: 'Assets', icon: <ArchiveBoxIcon />, permission: 'view:assets' },
