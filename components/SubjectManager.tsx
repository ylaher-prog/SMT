...
const SubjectManager: React.FC<SubjectManagerProps> = ({ academicStructure, onUpdateSubjects, currentTenantId }) => {
    ...
    const handleSaveSubject = (subjectData: Omit<Subject, 'id' | 'tenantId'>) => {
        if (subjectToEdit) {
    ...
    const handleExportSubjects = () => {
        ...
        const csvRows = subjects.map(s => {
            ...
            // FIX: Use curriculumIds and map to names for export
            const curricula = s.curriculumIds.map(id => academicStructure.curricula.find(c => c.id === id)?.name || id).join(';');
                        ...
                        <tr key={subject.id}>
                            ...
                            <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                               <div className="flex flex-wrap gap-1 max-w-xs">
                                    {/* FIX: Use curriculumIds and map to names */}
                                    {subject.curriculumIds.map(cId => <span key={cId} className="px-2 py-0.5 text-xs bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-gray-300 rounded-full">{curriculumMap.get(cId) || cId}</span>)}
                                </div>
                            </td>
                            <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                               <div className="flex flex-wrap gap-1 max-w-xs">
