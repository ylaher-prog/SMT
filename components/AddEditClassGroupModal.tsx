...
const AddEditClassGroupModal: React.FC<AddEditClassGroupModalProps> = ({ isOpen, onClose, academicStructure, setClassGroups, existingGroup, currentAcademicYear, currentTenantId }) => {
    ...
    useEffect(() => {
        if (existingGroup) {
            setFormData({
                ...
                curriculumId: existingGroup.curriculumId,
    ...
    const availableSubjects = useMemo(() => {
        if (!formData.grade || !formData.curriculumId) return [];
        return academicStructure.subjects.filter(s => 
            s.grades.includes(formData.grade) && 
            s.curriculumIds.includes(formData.curriculumId)
        );
    }, [formData.grade, formData.curriculumId, academicStructure.subjects]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const groupData: Omit<ClassGroup, 'id' | 'tenantId'> = {
            ...formData,
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <Fieldset legend="Group Details">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div>
                    <FormLabel htmlFor="curriculumId">Curriculum</FormLabel>
                    <FormSelect id="curriculumId" name="curriculumId" value={formData.curriculumId} onChange={handleChange}>
                         {academicStructure.curricula.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </FormSelect>
                    {errors.curriculumId && <p className="text-red-500 text-xs mt-1">{errors.curriculumId}</p>}
                </div>
                <div>
