...
import type { Teacher, AcademicStructure, TeacherWorkload, TeacherAllocation, ClassGroup, Subject, GeneralSettings, PhaseStructure, TimetableHistoryEntry, TimeGrid, GeneratedSlot, Curriculum } from '../types';
...
const WorkloadReportModal: React.FC<WorkloadReportModalProps> = (props) => {
    ...
    const involvedPhases = useMemo(() => {
        ...
        // FIX: Use curriculumId from ClassGroup
        const gradeCurricula = new Set(assignedClasses.map(ac => `${ac.group.grade}|${ac.group.curriculumId}`));
        
        gradeCurricula.forEach(gcPair => {
            const [grade, curriculumId] = gcPair.split('|');
            // FIX: use curriculumIds from PhaseStructure
            const phase = phaseStructures.find(p => p.grades.includes(grade) && p.curriculumIds.includes(curriculumId));
            if (phase) {
