...
import { FormLabel, FormInput, FormSelect, FormTextarea, Checkbox, Fieldset } from './FormControls';
...
const MonitoringSetup: React.FC<MonitoringSetupProps> = ({ monitoringTemplates, setMonitoringTemplates, phaseStructures, currentTenantId }) => {
    ...
    const handleUpdateTemplateDetails = (updates: Partial<MonitoringTemplate>) => {
        if (!activeTemplate) return;
        handleSaveTemplate({ ...activeTemplate, ...updates });
    };
    ...
    const handleDeleteTemplate = () => {
        if (activeTemplate) {
             if (window.confirm(`Are you sure you want to delete "${activeTemplate.name}"? This cannot be undone.`)) {
                setMonitoringTemplates(prev => prev.filter(t => t.id !== activeTemplate.id));
                const remainingTemplates = monitoringTemplates.filter(t => t.id !== activeTemplate.id);
    ...
    const handleAddField = (type: FormFieldType) => {
        ...
        const newField: FormField = {
            ...
            placeholder: '',
            weight: type === FormFieldType.Rating ? 1 : undefined,
        };
    ...
    const renderPropertiesPanel = () => {
        if (editingField) {
            return (
                <div className="p-3 border dark:border-slate-700 rounded-lg space-y-4 bg-white dark:bg-slate-800/50">
                    <h4 className="font-semibold text-sm border-b dark:border-slate-600 pb-2">Field Properties</h4>
                    ...
                    {editingField.type === FormFieldType.Rating && (
                        <div>
                            <FormLabel>Weight</FormLabel>
                            <FormInput type="number" value={editingField.weight || ''} onChange={e => handleUpdateField(editingField.id, { weight: parseInt(e.target.value) || 1 })} placeholder="Score Weight" />
                        </div>
                    )}
        ...
        if (activeTemplate) {
            return (
                 <div className="p-3 border dark:border-slate-700 rounded-lg space-y-4 bg-white dark:bg-slate-800/50">
                    <h4 className="font-semibold text-sm border-b dark:border-slate-600 pb-2">Form Properties</h4>
                    <div>
                        <FormLabel>Form Name</FormLabel>
                        <FormInput value={activeTemplate.name} onChange={e => handleUpdateTemplateDetails({ name: e.target.value })} placeholder="Form Name"/>
                    </div>
                    <div>
                        <FormLabel>Restrict to Phase</FormLabel>
                         <FormSelect value={activeTemplate.phaseId || ''} onChange={e => handleUpdateTemplateDetails({ phaseId: e.target.value || undefined })}>
            ...
            <div className="bg-white dark:bg-slate-800/50 p-6 rounded-xl shadow-sm">
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        {activeTemplate && (
                            <button onClick={handleDeleteTemplate} className="text-red-500 p-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full" title="Delete Template"><TrashIcon className="w-5 h-5"/></button>
                        )}
                    </div>
                    {activeTemplate && (
                        <div className="flex items-center gap-2">
