...
import type { Teacher, ClassGroup, TeacherAllocation, Subject, AcademicStructure, TeacherWorkload, AllocationSettings, GeneralSettings, PhaseStructure, TimeGrid, TimetableHistoryEntry, Permission, Curriculum } from '../types';
import { AllocationRole, AllocationStrategy, SubjectCategory } from '../types';
import { SparklesIcon, ArrowDownTrayIcon, XMarkIcon, TrashIcon, ClipboardDocumentListIcon, CheckCircleIcon, ExclamationTriangleIcon, ChartPieIcon } from './Icons';
...
const Allocations: React.FC<AllocationsProps> = (props) => {
    const { teachers, setTeachers, classGroups, allocations, academicStructure, phaseStructures, setAllocations, teacherWorkloads, allocationSettings, generalSettings, timeGrids, timetableHistory, permissions, logAction, currentTenantId } = props;
    ...
    const filteredClassGroups = useMemo(() => {
        return classGroups.filter(cg => 
            (filters.curricula.length === 0 || filters.curricula.includes(cg.curriculumId)) &&
    ...
    const summaryData = useMemo(() => {
        ...
        filteredClassGroups.forEach(group => {
            const visibleSubjectsForGroup = group.subjectIds.filter(sid => matrixSubjects.some(s => s.id === sid));
            totalSlots += visibleSubjectsForGroup.length;
        });

        const allocatedSlots = allocations.filter(alloc => {
            const isGroupVisible = filteredClassGroups.some(fcg => fcg.id === alloc.classGroupId);
            const isSubjectVisible = matrixSubjects.some(ms => ms.id === alloc.subjectId);
            return isGroupVisible && isSubjectVisible;
        }).length;

        const unallocatedSlots = totalSlots - allocatedSlots;
        const completionPercentage = totalSlots > 0 ? Math.round((allocatedSlots / totalSlots) * 100) : 0;
    ...
    }, [filteredClassGroups, matrixSubjects, allocations]);
    ...
    const handleUpdateAllocation = (classGroupId: string, subjectId: string, teacherId: string) => {
        setAllocations(prev => {
            ...
            // FIX: Add tenantId to new allocation to satisfy TeacherAllocation type.
    ...
    const handleAutoAllocate = () => {
        ...
        allocationsToCreate.sort((a, b) => getSubjectPeriods(b.subject, b.group.curriculumId, b.group.grade, b.group.mode) - getSubjectPeriods(a.subject, a.group.curriculumId, a.group.grade, a.group.mode));
        ...
        allocationsToCreate.forEach(({ group, subject }) => {
            ...
            if (qualifiedTeachers.length > 0) {
                const subjectPeriods = getSubjectPeriods(subject, group.curriculumId, group.grade, group.mode);
    ...
    const renderBuilder = () => (
        <div className="bg-white dark:bg-slate-800/50 p-4 sm:p-6 rounded-xl shadow-sm">
            <div className="flex flex-col sm:flex-row justify-between items-start mb-4">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-4 sm:mt-0">
                    ...
                    <button onClick={handleAutoAllocate} className="px-3 py-1.5 text-sm font-semibold text-white bg-brand-navy rounded-md hover:bg-slate-800 dark:bg-brand-primary dark:hover:bg-rose-900 flex items-center gap-2">
                        <SparklesIcon className="h-4 h-4" />
                        Auto-Allocate
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div className="bg-gray-50 dark:bg-slate-900/50 p-4 rounded-lg flex items-center">
                    <div className="p-3 rounded-full bg-blue-100 dark:bg-blue-900/50 mr-4">
                        <ClipboardDocumentListIcon className="h-6 w-6 text-blue-500 dark:text-blue-400" />
                    </div>
                    <div>
                        <p className="text-sm font-medium text-brand-text-light dark:text-gray-400">Total Slots</p>
                        <p className="text-2xl font-bold text-brand-text-dark dark:text-white">{summaryData.totalSlots}</p>
                    </div>
                </div>
                <div className="bg-gray-50 dark:bg-slate-900/50 p-4 rounded-lg flex items-center">
                    ...
                    <div>
                        <p className="text-sm font-medium text-brand-text-light dark:text-gray-400">Allocated</p>
                        <p className="text-2xl font-bold text-brand-text-dark dark:text-white">{summaryData.allocatedSlots}</p>
                    </div>
                </div>
                <div className="bg-gray-50 dark:bg-slate-900/50 p-4 rounded-lg flex items-center">
                    <div className="p-3 rounded-full bg-amber-100 dark:bg-amber-900/50 mr-4">
                        <ExclamationTriangleIcon className="h-6 w-6 text-amber-500 dark:text-amber-400" />
                    </div>
                    <div>
                        <p className="text-sm font-medium text-brand-text-light dark:text-gray-400">Unallocated</p>
                        <p className="text-2xl font-bold text-brand-text-dark dark:text-white">{summaryData.unallocatedSlots}</p>
                    </div>
                </div>
                <div className="bg-gray-50 dark:bg-slate-900/50 p-4 rounded-lg flex items-center">
                    <div className="p-3 rounded-full bg-indigo-100 dark:bg-indigo-900/50 mr-4">
                        <ChartPieIcon className="h-6 w-6 text-indigo-500 dark:text-indigo-400" />
                    </div>
                    <div>
                        <p className="text-sm font-medium text-brand-text-light dark:text-gray-400">Completion</p>
                        <p className="text-2xl font-bold text-brand-text-dark dark:text-white">{summaryData.completionPercentage}%</p>
                    </div>
                </div>
            </div>

             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 mb-4 p-3 border-y dark:border-slate-700">
                {/* FIX: Removed non-existent 'version' property from curriculum name. */}
                <MultiSelectFilter label="Curricula" options={curricula.map(c => ({ id: c.id, name: c.name }))} selected={filters.curricula} onChange={(s) => handleFilterChange('curricula', s)} />
                    ...
                    <tbody className="bg-white dark:bg-transparent divide-y divide-gray-200 dark:divide-slate-700">
                       {filteredClassGroups.map(group => {
                           const totalPeriods = group.subjectIds
                                ...
                                .reduce((sum, subject) => sum + (subject ? getSubjectPeriods(subject, group.curriculumId, group.grade, group.mode) : 0), 0);
                           return (
                               <tr key={group.id}>
                                   <td className="sticky left-0 bg-white dark:bg-slate-800 px-4 py-3 font-semibold text-gray-800 dark:text-gray-200 w-48 border-r dark:border-slate-700 z-10">
                                       <div>{group.name}</div>
                                       <div className="font-normal text-xs text-gray-500 dark:text-gray-400">{curricula.find(c => c.id === group.curriculumId)?.name || group.curriculumId}</div>
