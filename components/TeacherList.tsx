...
import type { Teacher, AcademicStructure, TeacherWorkload } from '../types';
import { EmploymentStatus } from '../types';
import { PencilIcon, TrashIcon, PlusIcon, ArrowsUpDownIcon, ArrowUpIcon, ArrowDownIcon, UserCircleIcon, DocumentChartBarIcon, KeyIcon, XMarkIcon } from './Icons';
...
interface TeacherListProps {
    ...
    permissions: any[]; // Using any for simplicity from props
...
const TeacherList: React.FC<TeacherListProps> = (props) => {
    const { teachers, setTeachers, academicStructure, workloads, permissions, initialFilters, onClearInitialFilters, onSelectTeacher, currentTenantId } = props;
    const [filters, setFilters] = useState({ name: '', status: 'all', position: '', workload: '' });
    ...
    const sortedAndFilteredTeachers = useMemo(() => {
        let filtered = teachers.filter(teacher => {
            ...
            return (
                (teacher.fullName.toLowerCase().includes(nameFilter) || 
                 teacher.email.toLowerCase().includes(nameFilter) ||
                 (teacher.employeeCode || '').toLowerCase().includes(nameFilter)
                ) &&
        ...
        if (filters.workload === 'overloaded') {
            filtered = filtered.filter(t => {
                ...
                return periodUtil > 1 || learnerUtil > 1;
            });
        }
        ...
        if (sortConfig !== null) {
            filtered.sort((a, b) => {
                ...
                switch (sortConfig.key) {
                    ...
                    case 'employmentStatus': aValue = a.employmentStatus; bValue = b.employmentStatus; break;
                    case 'position': aValue = positionMap.get(a.positionId) || ''; bValue = positionMap.get(b.positionId) || ''; break;
                    case 'periods': aValue = workloads.get(a.id)?.totalPeriods || 0; bValue = workloads.get(b.id)?.totalPeriods || 0; break;
                    case 'username': aValue = a.username || ''; bValue = b.username || ''; break;
                    default: return 0;
                }
    ...
    const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        setFilters(prev => ({ ...prev, [name]: value, workload: '' })); // Clear special filter on manual change
    };
        ...
        <>
            <div className="bg-white dark:bg-slate-800/50 p-6 rounded-xl shadow-sm">
                <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold text-brand-dark dark:text-white">Staff Directory ({sortedAndFilteredTeachers.length})</h3>
                    {hasPermission(permissions, 'add:teacher') && (<button onClick={() => setAddModalOpen(true)} className="bg-brand-primary text-white px-3 py-2 text-sm rounded-md flex items-center gap-1.5 font-medium hover:bg-rose-900 transition-colors"><PlusIcon className="w-4 h-4"/> Add Staff Member</button>)}
                </div>
                {filters.workload && (
                    <div className="bg-sky-100 dark:bg-sky-900/40 p-3 rounded-lg mb-4 flex items-center justify-between">
                        <span className="text-sm font-semibold text-sky-800 dark:text-sky-200">Filtering by: Overloaded Staff</span>
                        <button onClick={() => setFilters(prev => ({ ...prev, workload: '' }))} className="flex items-center gap-1 text-sm text-sky-700 dark:text-sky-300 hover:underline"><XMarkIcon className="w-4 h-4" />Clear</button>
                    </div>
                )}
                        ...
                        <tbody className="bg-white dark:bg-transparent divide-y divide-gray-200 dark:divide-slate-700">
                            {sortedAndFilteredTeachers.map(teacher => {
                                ...
                                <tr key={teacher.id} className="hover:bg-gray-50 dark:hover:bg-slate-800">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 w-48"><div>Periods: {workload?.totalPeriods || 0} / {totalMaxPeriods}</div><WorkloadBar current={workload?.totalPeriods || 0} max={totalMaxPeriods} /><div className="mt-1">Learners: {workload?.totalLearners || 0} / {teacher.maxLearners}</div><WorkloadBar current={workload?.totalLearners || 0} max={teacher.maxLearners} /></td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium"><div className="flex items-center space-x-3">{hasPermission(permissions, 'edit:teacher') && <button onClick={() => setTeacherToEdit(teacher)} className="text-brand-accent hover:text-amber-700"><PencilIcon className="h-5 w-5"/></button>}{hasPermission(permissions, 'delete:teacher') && <button onClick={() => setTeacherToDelete(teacher)} className="text-red-600 hover:text-red-800"><TrashIcon className="h-5 w-5"/></button>}{hasPermission(permissions, 'view:teacher-report') && <button onClick={() => onSelectTeacher(teacher)} className="text-brand-primary hover:text-rose-800"><DocumentChartBarIcon className="h-5 w-5"/></button>}{hasPermission(permissions, 'reset:teacher-password') && <button onClick={() => handleResetPassword(teacher)} className="text-gray-500 hover:text-brand-navy dark:hover:text-gray-200"><KeyIcon className="h-5 w-5"/></button>}</div></td>
                                </tr>
                            )})}
                        </tbody>
                    </table>
            ...
            {isAddModalOpen && <AddEditTeacherModal isOpen={isAddModalOpen} onClose={() => setAddModalOpen(false)} setTeachers={setTeachers} academicStructure={academicStructure} teachers={teachers} currentTenantId={currentTenantId} />}
            {teacherToEdit && <AddEditTeacherModal isOpen={!!teacherToEdit} onClose={() => setTeacherToEdit(null)} setTeachers={setTeachers} existingTeacher={teacherToEdit} academicStructure={academicStructure} teachers={teachers} currentTenantId={currentTenantId} />}
