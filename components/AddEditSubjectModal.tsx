...
import type { Subject, AcademicStructure, Curriculum } from '../types';
...
interface AddEditSubjectModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (subjectData: Omit<Subject, 'id' | 'tenantId'>) => void;
...
export const AddEditSubjectModal: React.FC<AddEditSubjectModalProps> = ({ isOpen, onClose, onSave, existingSubject, academicStructure }) => {
    ...
    type Override = { curriculumId: string; grade: string; mode: string; periods: number };
    ...
    useEffect(() => {
        if (existingSubject) {
            setFormData({
                ...
                curriculumIds: existingSubject.curriculumIds || [],
                periodsByMode: existingSubject.periodsByMode || [],
                periodOverrides: (existingSubject.periodOverrides as Override[]) || [],
    ...
    const handleMultiSelectChange = (field: 'grades' | 'curriculumIds', value: string) => {
        setFormData(prev => {
    ...
    const handleAddOverride = () => {
        setFormData(prev => ({
            ...
            periodOverrides: [
                ...
                { curriculumId: academicStructure.curricula[0]?.id || '', grade: academicStructure.grades[0] || '', mode: academicStructure.modes[0] || '', periods: 1 }
            ]
        }));
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // FIX: Add tenantId to the saved object to satisfy the Subject type. Note that the onSave prop is updated to expect Omit<Subject, 'id' | 'tenantId'>.
        onSave({ 
            name: formData.name,
    ...
    const CheckboxGroup: React.FC<{ title: string; items: {id: string, name: string}[]; selectedItems: string[]; onChange: (item: string) => void; }> = ({ title, items, selectedItems, onChange }) => (
         <div>
            ...
            <div className="mt-2 p-3 border border-gray-300 dark:border-slate-600 rounded-md max-h-40 overflow-y-auto">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {items.map(item => (
                        <label key={item.id} className="flex items-center space-x-2 text-sm cursor-pointer dark:text-gray-300">
                            <input
                                type="checkbox"
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={existingSubject ? "Edit Subject" : "Add New Subject"} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* FIX: Corrected typo from 'curriculaIds' to 'curriculumIds'. */}
        <CheckboxGroup 
            title="Applicable Curricula"
            items={academicStructure.curricula.map(c => ({id: c.id, name: c.name}))}
            selectedItems={formData.curriculumIds}
            onChange={(curriculumId) => handleMultiSelectChange('curriculumIds', curriculumId)}
        />
        
        <CheckboxGroup 
            title="Applicable Grades"
            items={academicStructure.grades.map(g => ({id: g, name: g}))}
            ...
            <div className="mt-2 space-y-2">
                {formData.periodOverrides.map((override, index) => (
                    <div key={index} className="grid grid-cols-5 items-center gap-2 p-2 bg-gray-50 dark:bg-slate-700/50 rounded-md">
                        <select value={override.curriculumId} onChange={(e) => handleUpdateOverride(index, 'curriculumId', e.target.value)} className="w-full border-gray-300 dark:border-slate-600 rounded-md shadow-sm sm:text-sm bg-transparent dark:bg-slate-800 col-span-2">
