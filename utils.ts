...
export const getSubjectPeriods = (subject: Subject, curriculumId: string, grade: string, mode: string): number => {
    // FIX: Use curriculumId to find period overrides
    const override = subject.periodOverrides?.find(o => o.curriculumId === curriculumId && o.grade === grade && o.mode === mode);
    if (override) {
